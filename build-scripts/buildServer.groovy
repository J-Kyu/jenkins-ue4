def _BUILD_PATH = "D:\\jenkins_build_scripts\\jenkins-ue4-JLinux\\build-scripts"
...
pipeline {
    ...
    stages {
        stage('Perfroce Sync') {
            steps(){
                ...
                p4sync( 
					credential: 'vr_build',
					format: 'jenkins-Server-VR_Amolang',
					populate: syncOnly(
					    have: true,
					    quiet: true
					    ),
    ...
    post{
        success{
            archiveArtifacts artifacts: 'VR_Multiplay\\Build\\LinuxServer.tar', fingerprint: true
            dir("${_BUILD_PATH}"){
                bat "Step6_UploadToFilePort.bat"
            }
